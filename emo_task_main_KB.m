...
stim_loc{4} = CenterRectOnPoint(r, window_rect(3)*0.6, window_rect(4)*0.3);
stim_loc{5} = CenterRectOnPoint(r, window_rect(3)*0.7, window_rect(4)*0.3);
stim_loc{1} = CenterRectOnPoint(r, window_rect(3)*0.3, window_rect(4)*0.3);
stim_loc{2} = CenterRectOnPoint(r, window_rect(3)*0.4, window_rect(4)*0.3);
...
n_trial = numel(ts.emo_order{start_input.run_num});
[~,~,mathprob] = xlsread(fullfile(basedir, 'Maths_stimuli.xlsx'));
math_stim = mathprob(2:n_trial+1,6)';
math_anws = cell2mat(mathprob(2:n_trial+1,7))';
randomizer = randperm(n_trial,n_trial);
...
data.trial_sequence.math_order = math_stim;

for block = 1%:n_trial % block: per emotion category
    ...
    for n = 1:length(stim_loc)
        ...
        while ii <= n
            DrawFormattedText(theWindow, cnum{ii}, stim_loc{ii}(3), stim_loc{ii}(4), black);
            ii = ii + 1;
        end
