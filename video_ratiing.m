...
for trial = numel(stim_order) % repeat for 140 trials (trial: per one video)
    ...
    % ?
    data.dat{trial}.emotion_category = video_cat{emotion_num};
    data.dat{trial}.target_stim_number = stim_num;
    data.dat{trial}.target_filename = target_file;
    ...
    data.dat{trial}.stim_start_time = GetSecs;

    while GetSecs-data.dat{trial}.stim_start_time < 5 %(~done) %~KbCheck
        % Wait for next movie frame, retrieve texture handle to it
        tex = Screen('GetMovieImage', theWindow, moviePtr);

        if tex<=0
            waitsec_fromstarttime(data.dat{trial}.stim_start_time,5);
            break;
        end
    ...
    data.dat{trial}.stim_end_time = GetSecs;
